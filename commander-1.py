...
class ResponseParser:
    ...
    def parse_response(self, response: str) -> Dict[str, str]:
        ...
        # Clean up the response - remove any wrapper blocks like ```tool_code
        cleaned_response = response
        
        # Remove ```tool_code wrapper if present
        if '```tool_code' in cleaned_response:
            # Extract content between ```tool_code and the final ```
...
if __name__ == "__main__":
    main()
            ...
            tool_code_match = re.search(tool_code_pattern, cleaned_response, re.DOTALL)
            if tool_code_match:
                cleaned_response = tool_code_match.group(1)
        
        # Pattern to match file blocks: ---filename--- followed by ```language code ```
        # Made more flexible to handle different languages and formatting
        pattern = r'---([^-\n]+)---\s*\n```(?:\w+)?\s*\n(.*?)\n```'
        matches = re.findall(pattern, cleaned_response, re.DOTALL)
        
        if not matches:
            # Try alternative pattern without newline after filename
            pattern = r'---([^-\n]+)---\s*```(?:\w+)?\s*\n(.*?)\n```'
            matches = re.findall(pattern, cleaned_response, re.DOTALL)
        ...
        # Debug output if no files were parsed
        if not modified_files:
            print("🐛 Debug: No files parsed from response")
            print(f"Response length: {len(response)} characters")
            print("First 500 characters of cleaned response:")
            print(cleaned_response[:500])
            print("---")
            
            # Try to show what patterns we're looking for
